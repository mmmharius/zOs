...
void debug_print_state(unsigned char sc) {
    ...
    /* history: store the printable char, fallback to '?' */
    ...
    /* clear first 8 rows of debug screen */
    dbg->row = 0;
    dbg->col = 0;
    ...
    /* row 5 — buffer usage of current screen */
...
/*
   PRINT [id] [max_rows]
   Copies rows 0..max_rows of screen[id] into debug screen rows 8..
   Then opens split with debug screen on the right.

   id       : 0-3  (default: scr.current)
   max_rows : 0-24 (default: VGA_HEIGHT)
*/
...
void debug_live_print(int id, int max_rows) {
    ...
    /* open split with debug on right if not already open */
